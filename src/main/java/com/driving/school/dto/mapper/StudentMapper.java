 ...
 * <br>
 * This abstract class will be automagically extended by MapStruct. Only the 'abstract' methods are auto-generated,
 * the concrete implementations are needed for type-casting.
 */
...
public abstract class StudentMapper implements GenericMapper<Student> {
     ...
     * convert it to a new Student entity.<br><br>
     * The cast is required because MapStruct is a compile-time processor and is not aware of run-time types, so it
     * can't perform the cast dynamically. It could be enforced by the @SubclassMapping annotation, but sadly that's
     * not supported for Update methods, so instead this approach was chosen for consistency.
     * @param   requestData
     *          <code>StudentCreationDto</code> source of data
     * @return
     *          a new <code>Student</code> entity
     */
    @Override
    ...
    /**
     * Casts the requestData to the appropriate DTO type and then calls an autogenerated MapStruct mapping method to
     * overwrite the fields of the Student destination, without creating a new entity. Any <code>null</code>
     * values in the requestData will be skipped.<br><br>
     * The cast is required because MapStruct is a compile-time processor and is not aware of run-time types,
     * so it can't perform the cast dynamically. Note that @SubclassMapping is not supported for Update methods.
     * @param   destination
     *          a Student entity that should be modified
     * @param   requestData
     *          a StudentUpdateDto with some (or all) updated field values
     */
    @Override
